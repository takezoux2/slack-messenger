...
export class Configuration {
  ...
  isValidNodeVersion(): boolean {
    // Check if version follows proper format like ">=22.0.0"
    const versionRegex = /^>=\d+\.\d+\.\d+$/
    return versionRegex.test(this.nodeVersion) && this.isNodeVersion22OrHigher()
  }
  ...
  static createDefault(): Configuration {
    return new Configuration('>=22.0.0', true, 'vitest', 'dist')
  }
  ...
  private validateProperties(): void {
    if (!this.isNodeVersion22OrHigher()) {
      throw new Error('nodeVersion must specify Node.js 22 or higher')
    }
   ...
   */
  private isNodeVersion22OrHigher(): boolean {
    // Extract major version from string like ">=22.0.0"
