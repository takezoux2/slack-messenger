...
describe('Configuration Entity', () => {
  ...
  beforeEach(() => {
    ...
    config = new Configuration('>=22.0.0', true, 'vitest', 'dist')
  })
  ...
  describe('constructor', () => {
    ...
    it('should throw error for Node.js version below 22', () => {
      expect(() => {
        new Configuration('>=21.0.0', true, 'vitest', 'dist')
      }).toThrow('nodeVersion must specify Node.js 22 or higher')
    })

    it('should throw error when strict mode is false', () => {
      expect(() => {
        new Configuration('>=22.0.0', false, 'vitest', 'dist')
      }).toThrow('strict must be true (constitutional requirement)')
    })

    it('should throw error for non-vitest test framework', () => {
      expect(() => {
        new Configuration('>=22.0.0', true, 'jest', 'dist')
      }).toThrow('testFramework must be "vitest"')
    })

    it('should throw error for non-dist build target', () => {
      expect(() => {
        new Configuration('>=22.0.0', true, 'vitest', 'build')
  ...
  describe('validation', () => {
    ...
    it('should reject invalid version formats', () => {
      ...
      expect(() => {
        new Configuration('22', true, 'vitest', 'dist')
      }).toThrow('nodeVersion must specify Node.js 22 or higher')
    })
  ...
  describe('static factory methods', () => {
    it('should create default configuration', () => {
      ...
      expect(defaultConfig.nodeVersion).toBe('>=22.0.0')
